...
class AsciiParseUnitTest(unittest.TestCase):
    ...
    def test_read_until(self):
        ...
        self.w.readUntil("z", "\\")
        self.assertEqual(
            self.w.readEOF(), True, "failed in readUntil : [%d]" % self.i()
        )
        self.assertEqual(self.i(), 6, "failed in readUntil : [%d]" % self.i())
    ...
    def test_save_and_restore_context(self):
        ...
        self.w.restoreContext()
        self.assertEqual(
            self.i(), 0, "failed while saving/restoring context : [%d]" % self.i()
        )
    ...
    def test_capture(self):
        ...
        self.assertEqual(res, "abc", "failed while extract by tag : [%d]" % self.i())
